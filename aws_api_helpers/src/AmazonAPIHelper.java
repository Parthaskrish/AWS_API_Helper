...
/**
 * This class helps with calling Amazon API's. The class uses Amazon's sdk for the calls.
 **/
public class AmazonAPIHelper {
    //Assign values from the test classes or a configuration file
    public String ACCESS_KEY_ID = null;//Add your subscription's access key here
    public String SECRET_KEY = null;//Add your subscription's secret key here
    ...
    public String AMI_IMAGE_ID = "ami-22ce4934";
     ...
     * @param maxNumOfInstances The maximum number of instances to launch.
    ...
    public List<String> createEC2Instance(AmazonEC2 ec2, String instanceName, String amiImageId, String type, String keyPairName, int minNumOfInstances, int maxNumOfInstaces, String subnetID, List<Tag> tags) {
        ...
        try{
            ...
            Log.error("EXCEPTION FOUND createEC2Instance: \n"+ase);
	...
	/* ****************************************************************************** */
    /**
     * This method deletes all the amazon instances associated with the given VDC
     * @param vdcName Name of the VDC from where all the instances need to be deleted
     * @param ec2 Amazon client of the account from where the instances are to be deleted
     * */
    public void deleteAmazonInstances(String vdcName, AmazonEC2 client){

        try{
            DBConnectionsHelper dbHelper = new DBConnectionsHelper();
            String host = dbHelper.getDBHost();
            String userName = dbHelper.getDBUserName();
            String password = dbHelper.getDBPassword();
            Connection connection = dbHelper.connectToDB(host, userName, password);
            List<String> instanceIDs = dbHelper.fetchInstanceID(connection, vdcName);
            deleteEC2Instance(client, instanceIDs);
        }
        catch (Exception e){
            Log.error("EXCEPTION FOUND in deleteAmazonInstances: \n"+e);
        }
    }
     ...
     * @param ec2 Amazon client of the account from where the instances are to be deleted
     * */
    public void deleteAmazonInstances(String host, String userName, String password, String vdcName, AmazonEC2 client){
